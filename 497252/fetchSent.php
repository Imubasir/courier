...
if($origin != '' && $pay_method != '' && $date_from != '' && $date_to != '') {
	...
	LEFT JOIN courier_payments ON courier_parcel.parcel_code = courier_payments.parcel_code_fk
	WHERE
	    sender_origin = '$origin' ORDER BY manifest_date_added ASC";
} 
	...
	LEFT JOIN courier_payments ON courier_parcel.parcel_code = courier_payments.parcel_code_fk
	WHERE
	    pay_method = '$pay_method' ORDER BY manifest_date_added ASC";
}
	...
	LEFT JOIN courier_payments ON courier_parcel.parcel_code = courier_payments.parcel_code_fk
	WHERE
	    date_created BETWEEN '$date_from' AND DATE_ADD('$date_to', INTERVAL 1 DAY) ORDER BY manifest_date_added ASC";
}
