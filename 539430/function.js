		...
		success: function (json) {
			...
			if(data.length != 0) {
				...
				for (var i = 0, a = 1; i < data.length, a < data.length+1; i++, a++) {
					...
					var action = "<button class='btn btn-sm btn-info' onclick='viewDetails(\""+parcel_code+"\")'><i class='fa fa-ellipsis-v'></i> Details</button>";

					tbl_body += "<tr><td>"+a+"</td><td>"+date_created+"</td><td>"+parcel_code+"</td><td>"+sender_name+"</td><td>"+recipient_name+"</td><td>"+sender_origin+"</td><td>"+recipient_destination+"</td><td>"+action+"</td></tr>";
				}
