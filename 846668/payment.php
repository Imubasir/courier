...
$pay_check = "SELECT * FROM courier_billings WHERE bill_parcel_code = '$p_code' AND bill_type = 'OVERDUE'";
...
if($pay_rs->num_rows > 0) {
	...
	while($row = mysqli_fetch_assoc($pay_rs)) {
		if($row['bill_clear'] == '0') {
			$pay_query = "INSERT INTO courier_payments (parcel_code_fk, pay_type, pay_method, received_by, payment_branch) VALUES ('$p_code', 'OVERDUE', '$pay_method', '$username', '$location')";

			$rs = mysqli_query($link, $pay_query);
			if($rs) {

				$update_bill = "UPDATE courier_billings SET bill_clear = '1' WHERE bill_parcel_code = '$p_code' AND bill_type = 'OVERDUE'";
